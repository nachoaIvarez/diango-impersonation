...
class ViewTest(TestCase):
    ...
    def get_target_url(self, target_user=None):
        ...
        response = self.client.get(reverse(
            "impersonate_user", kwargs={'pk': target_user.id,
                                        'user_model': (target_user
                                                       .__class__
                                                       .__name__)}))
    ...
    @override_settings(CAN_IMPERSONATE=login_as_nonstaff)
    def test_custom_permissions(self):
        user = create_user("user", "pass", is_superuser=False, is_staff=False)
        staff1 = create_user("staff", "pass", is_superuser=False,
                             is_staff=True)
    ...
    @override_settings(CAN_IMPERSONATE='impersonation.tests.'
                                       'login_as_shorter_username')
    ...
    def test_custom_permissions_invalid_path(self):
        ...
        with override_settings(CAN_IMPERSONATE='impersonation.tests'
                                               '.invalid_func'):
            assertMessage("Module impersonation.tests "
                          "does not define a invalid_func function.")
        with override_settings(CAN_IMPERSONATE='impersonation.tests'
                                               '.invalid_path.func'):
            assertMessage("Error importing CAN_IMPERSONATE function.")
